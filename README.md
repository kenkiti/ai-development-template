...
1. このリポジトリを土台にして、新しいプロジェクト用のGitリポジトリを作ります。GitHub上で使う場合は、リポジトリの **Settings** で **Template repository** を有効にすると、**Use this template** ボタンからファイル一式をコピーした新しいリポジトリを作れます。GitHubの設定に慣れていない場合は、このリポジトリのファイルをパソコンにコピーして、新しいフォルダーで `git init` を実行し、GitHubで新しい空のリポジトリを作ってから、そのリポジトリへアップロードしてもかまいません。どちらの方法でも、元のテンプレート自体は変更せず、新しく作ったリポジトリをプロジェクトとして使います。
